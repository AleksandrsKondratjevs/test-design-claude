    ...
    return Abstract.extend({
        ...
        initObservable: function () {
            return this._super().observe(
                "value totalProductCount totalDisabledProducts totalNotVisibleProducts loading"
            );
        },
        ...
        updateProductTotals: _.debounce(function () {
            ...
            if (
                this.conditionOption === "category_ids" &&
                typeof this.formData["category_ids"] !== "string"
            ) {
            ...
            if (!this.formData["conditions_encoded"]) {
                return;
            }
            ...
            this.jqXHR = $.ajax({
                ...
                .done(
                    function (response) {
                        ...
                        this.totalDisabledProducts(
                            parseInt(response.disabled, 10)
                        );
                        this.totalNotVisibleProducts(
                            parseInt(response.notVisible, 10)
                        );
                        totalText = this.valuePlaceholder.replace(
                            "%1",
                        ...
                        if (parseInt(response.disabled, 10) > 0) {
                            negativeTotals.push(
                                this.disabledPlaceholder.replace(
                                    "%1",
                                    parseInt(response.disabled, 10)
                                )
                            );
                        ...
                        if (parseInt(response.notVisible, 10) > 0) {
                            negativeTotals.push(
                                this.notVisiblePlaceholder.replace(
                                    "%1",
                                    parseInt(response.notVisible, 10)
                                )
                            );
